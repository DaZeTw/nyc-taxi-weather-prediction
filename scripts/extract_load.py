...
def extract_load(endpoint_url, access_key, secret_key):
    ...
    weather_data_cfg = cfg["weather_data"]
    ...
    for year in YEARS:
        ...
        for fp in all_fps:
            ...
            client_minio.fput_object(
                ...
                object_name=f"{datalake_cfg['taxi_folder_name']}/{os.path.basename(fp)}",
                file_path=fp,
            )

        # Upload weather files
        all_fps = glob(os.path.join(weather_data_cfg["folder_path"], year, "*.parquet"))
        for fp in all_fps:
            print(f"Uploading {fp}")
            client_minio = client.create_conn()
            client_minio.fput_object(
                bucket_name=datalake_cfg["bucket_name_1"],
                object_name=f"{datalake_cfg['weather_folder_name']}/{os.path.basename(fp)}",
                file_path=fp,
            )
###############################################
